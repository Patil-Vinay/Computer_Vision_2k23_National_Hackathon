...
def _display_detected_frames(conf, model, st_frame, image, is_display_tracking=None, tracker=None):
    ...
    for cls,box,track_id in zip(classes,boxes,track_ids):
            ...
            print(f"Track ID {track_id} history length: {len(track_history[track_id])}")
            if len(track) > 30:  # retain 90 tracks for 90 frames
                track.pop(0)
            points = np.hstack(track).astype(np.int32).reshape((-1, 1, 2))
            res_plot=cv2.polylines(res_plotted, [points], isClosed=False, color=(230, 230, 230), thickness=10)

            if class_name=="person":
                count = count+1
                print(count)
            
    st_frame.image(res_plot,
